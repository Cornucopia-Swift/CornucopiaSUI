...
cd Demo
xcodegen generate
set -o pipefail && xcodebuild -project CornucopiaSUIDemo.xcodeproj -scheme CornucopiaSUIDemo -destination 'platform=iOS Simulator,name=iPhone 17 Pro Max' build | xcbeautify -qq
```
...
- `CC_presentationDetentAutoHeight` follows the same native-sheet strategy as Daniel Saidi's 2026 size-to-fit sheet article: measure content height and feed it into a `.height(...)` presentation detent. Prefer improving this modifier, not replacing `CC_slideOverCard`, for native `.sheet` use. Good future adjustments: allow additional native detents such as `.medium`/`.large`, keep background interaction out of the height-only default unless explicitly requested, avoid empty detent sets before the first measurement, and use a small update threshold (about 0.5-1 pt) so real content-height changes track accurately. Keep `CC_slideOverCard` as the custom setup/pairing/permission card surface with its own dimming, drag, surface styling, safe-area behavior, and animated content resizing.
...
- **Never combine a matching field and keyboard input.** `VINTextField` + `VINKeyboardInput`, `NetworkAwareTextField` + `IPv4KeyboardInput`, and `NetworkAwareTextField` + `MACKeyboardInput` are invalid compositions for the same value. A `*KeyboardInput` is not a keyboard accessory for a field; it is the complete domain input: visible value display, focus target, normalization boundary, custom keypad, hardware-keyboard bridge, paste path, and submit surface. Pairing it with a matching `*TextField` creates two competing controls for one binding, duplicates validation, and leaves the keypad stranded inside arbitrary view layout instead of behaving like an input method. If a product wants an OS-positioned VIN keyboard, use `VINKeyboardTextField`, which attaches `VINKeyboardInput` as a real UIKit `inputView`. For other domains, build an equivalent real input method attached to the field. Do not stack or colocate a `*TextField` and its `*KeyboardInput` counterpart.
